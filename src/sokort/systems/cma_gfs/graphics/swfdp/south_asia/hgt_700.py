...
class SouthAsiaHgt700Plotter(SwfdpPlotter):
    ...
    def __init__(self, task: dict, work_dir: str, config: dict, **kwargs):
        super(SouthAsiaHgt700Plotter, self).__init__(task, work_dir, config, **kwargs)

        self.ncl_script_name = "swfdp_sa_hgt_700.ncl"

    def get_image_list(self):
        forecast_hour = f"{get_forecast_hour(self.forecast_timedelta):03}"

        return [{
            "path": f"./HGT_700hPa_{self.start_time}{forecast_hour}_SWFDP_SA.png"
        }]
